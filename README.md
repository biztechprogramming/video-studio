...
Hacker News, Reddit — into a narrated YouTube video: a stats card for each
subject, then a live scroll through its actual page, joined with an intro and
outro and uploaded with chapters and a thumbnail.
...
query ─► research ─► script ─► render ─► publish
         ...
         │           │          └─ cards (Chromium screenshot → Ken Burns clip)
...
- **OPENAI_API_KEY** — writes the script and speaks it. Already in `.env`.
...
- No B-roll, no talking head, no zoom-to-code-block choreography — the visual
  vocabulary is cards plus honest page scrolls.
